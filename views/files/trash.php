...
function restoreItem(id) {
    ...
    apiPost('/api/restore', { ids: [id] }, function() { location.reload(); });
}
function forceDeleteItem(id) {
    if (!confirm('彻底删除后无法恢复，确定要删除吗？')) return;
    apiPost('/api/force-delete', { ids: [id] }, function() { location.reload(); });
}
function batchRestore() {
    const ids = getSelectedIds();
    if (!ids.length) { alert('请选择文件'); return; }
    if (!confirm('确定要恢复选中的文件吗？')) return;
    apiPost('/api/restore', { ids: ids }, function() { location.reload(); });
}
function batchForceDelete() {
    const ids = getSelectedIds();
    if (!ids.length) { alert('请选择文件'); return; }
    if (!confirm('彻底删除后无法恢复，确定要删除吗？')) return;
    apiPost('/api/force-delete', { ids: ids }, function() { location.reload(); });
}
</script>
</body>
</html>